...
const Video = () => {
  ...
  const handleWatchLater = async () => {
    if (currentUser) {
      try {
        const res = await axios.put(`https://vid-stream-frontend.vercel.app/api/users/updatewatch/${path}`);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const videoRes = await axios.get(`https://vid-stream-frontend.vercel.app/api/videos/find/${path}`);
        const channelRes = await axios.get(
          `https://vid-stream-frontend.vercel.app/api/users/find/${videoRes.data.userId}`
        );
  ...
  const handleLike = async () => {
    if (currentUser) {
      await axios.put(`https://vid-stream-frontend.vercel.app/api/users/like/${currentVideo._id}`);
      if (currentUser._id) {
  ...
  const handleDislike = async () => {
    if (currentUser) {
      await axios.put(`https://vid-stream-frontend.vercel.app/api/users/dislike/${currentVideo._id}`);
      if (currentUser._id) {
  ...
  const handleSub = async () => {
    if (currentUser) {
      try {
        ...
        if (currentUser.subscribedUsers.includes(channel._id)) {
          await axios.put(`https://vid-stream-frontend.vercel.app/api/users/unsub/${channel._id}`);
          updatedSubscribers -= 1;
        } else {
          await axios.put(`https://vid-stream-frontend.vercel.app/api/users/sub/${channel._id}`);
          updatedSubscribers += 1;
        }
